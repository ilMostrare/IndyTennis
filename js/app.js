...
function setBindings() {
    ...
    $("form #TDScoreSubmit").click(function (evt) {
        ...
        if(!(TDMatchID)){
            swal("Oops...", "Please select a match!", "error");
        } else if ((TDSet1T1 > 7) || isNaN(TDSet1T1)){
            swal("Oops...", "Score Entered is Invalid!", "error");
    ...
    $("form #newTDTSubmit").click(function (evt){
        ...
        var userNewTDID1 = parseInt($("#userNewTDID1").val());
        var userNewTDID2 = parseInt($("#userNewTDID2").val());
        var newTDPoints = $("#newTDPoints").val();

        //#region Value Handling
        if(newTDPoints == ''){
            newTDPoints = 0;
        } else {
            newTDPoints = parseInt(newTDPoints);
        };
        //#endregion
        ...
        if (!(userNewTDID1 > 0)){
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    ntrUserNewTDID1: userNewTDID1,
                    ntrUserNewTDID2: userNewTDID2,
                    ntrNewTDPoints: newTDPoints
                }
            }).done(function (data) {
                //console.log("Success");
                console.log(data);
